...
## 7. Custom UI Styling & Themes

Authelia compiles its web frontend assets directly into the Go binary executable (`embed`), so static HTML files do not reside on disk inside the container.

### Built-in Native Dark Theme (`configuration.yml`)

Authelia supports built-in themes directly in `/opt/authelia/config/configuration.yml`:

```yaml
theme: dark # Options: default, dark, light, grey
```

### Bespoke Frontend UI Replacement (Recommended for Full Custom CSS)

Because `Barebones-Reverse-Proxy` is decoupled from Authelia via the `AuthProvider` Rust trait, you can host your own custom HTML/CSS/JS login page on `https://auth.bigiron.dev/` that calls Authelia's REST APIs (`/api/firstfactor` and `/api/secondfactor/webauthn`), giving you 100% control over design, animations, glassmorphism, and branding!
