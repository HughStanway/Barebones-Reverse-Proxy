...
* **Non-Blocking Async Cache Insertion**: On a Cache MISS, the proxy dispatches response headers and body bytes to the client **immediately**, spawning a background `tokio::spawn` task for LRU insertion, memory budgeting, and log emission. This eliminates the ~0.5ms latency penalty on initial requests.
