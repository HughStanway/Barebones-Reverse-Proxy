...
loop {
    ...
    tokio::task::spawn_local(async move {
        // ... TLS handshake and HTTP serving ...
    });
}
```
...
2.  **`spawn_local`**: Once a connection is accepted, a new "green thread" (task) is created. 
3.  **Concurrency**: Because each connection is handled in its own `spawn_local` task, the worker can handle thousands of concurrent connections. While one task is waiting for an upstream response, the loop can accept next connection, or other active tasks can make progress.
...
- It uses the `ProxyState` (which contains the `Router` and the upstream `Client`) to handle the request.
